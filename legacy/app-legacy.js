...
function statTargetCard(text) {
  ...
  for (const sec of sections) {
    ...
    for (const r of sec.rows) {
      if (r.label) {
        const hl = tileIdx === 0 ? ' hl' : '';
        body += `<div class="st-tile${hl}"><span class="st-label">${escHtml(r.label)}</span><span class="st-value">${escHtml(r.value)}</span></div>`;
        tileIdx++;
      } else {
        body += `<div class="st-full">${escHtml(r.value)}</div>`;
      }
    }
    body += '</div>';
  }
...
function clearLensViews() {
  // "Roster" lens: return to the dashboard from any view (tier or character)
  document.body.classList.remove('viewing-char');
  $('back-btn').classList.add('hidden');
  $('tier-view').classList.add('hidden');
  $('tier-btn').classList.remove('active');
